...
class Hyperparameters:
    ...
    fuse_batch_transfer = bool(int(os.environ.get("FUSE_BATCH_TRANSFER", "1")))
...
class DistributedTokenLoader:
    ...
    def __init__(
        ...
        world_size: int,
        device: torch.device,
        fuse_batch_transfer: bool = False,
    ):
        self.rank = rank
        self.world_size = world_size
        self.device = device
        self.fuse_batch_transfer = fuse_batch_transfer
    ...
    def next_batch(
        ...
        if self.fuse_batch_transfer:
            local = local.to(self.device, non_blocking=True)
            if local.dtype != torch.int64:
...
class CausalSelfAttention(nn.Module):
    def __init__(
        ...
        if attention_impl not in {"sdpa", "fa3"}:
            raise ValueError(f"Unsupported ATTENTION_IMPL={attention_impl}")
        if attention_impl == "fa3" and flash_attn_3_func is None:
            raise ValueError("ATTENTION_IMPL=fa3 requires flash-attn to be installed")
...
def main() -> None:
    ...
    train_loader = DistributedTokenLoader(
        ...
        world_size,
        device,
        fuse_batch_transfer=args.fuse_batch_transfer,
    )
